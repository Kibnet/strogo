...
| **Modules v0.2 / E05 (в работе)** | Строгий composable AST для records, bounded sequences, локальных вызовов и nested `if`; canonical source → typed deterministic IR | 104 parser/type/identity checks; runtime `if`, `fold`, proof и исполняемая библиотека ещё не готовы |

Это результаты сохранённых запусков и их отчётов: [Reserve v0](REPORT.md), [TaskGraph E04](artifacts/e04/REPORT.md) и [E05 nested-if checkpoint](artifacts/e05/REPORT.md). Они не являются результатом CI новой публичной копии. Происхождение публикации и границы сохранённых свидетельств описаны в [publication.md](docs/publication.md).
