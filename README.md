...
| **Modules v0.2 / E05 fold v0.1** | Строгий composable AST, owner bundle v0.4 и bounded left `fold`; canonical source → typed IR/reference execution → Dafny/C# с exact owner-prefix proof | 362 checks PASS; A/B=`Verified`, C=`Unproven(initial)`; две allocation-реализации доказаны и исполнены generated .NET consumer; admission и package ещё не готовы |

Это результаты сохранённых запусков и их отчётов: [Reserve v0](REPORT.md), [TaskGraph E04](artifacts/e04/REPORT.md), [E05 owner-composite v0.3](artifacts/e05/REPORT.md) и [fold v0.1](artifacts/e05/fold-v0.1-134856f/REPORT.md). Они не являются результатом CI новой публичной копии. Чистые Windows checkouts независимо от рабочего дерева воспроизвели 276-check owner-composite, 360-check fold implementation и итоговый 362-check conformance; для последнего два полных generated output sets совпали по путям и SHA-256. Это полезные внешние репликации, но не независимый third-party audit. Fold proof/runtime matrix выполнена официальным Dafny 4.11.0 под Ubuntu 24.04 WSL2: строгий oracle отличает доказательство, ожидаемый proof refusal и operational failure. Отдельный native Linux host, CI, package/admission, переносимый process-control harness и Linux ReadyToRun остаются непроверенными. Происхождение публикации и границы свидетельств описаны в [publication.md](docs/publication.md) и [журнале знаний](docs/knowledge-log.md).
...
Текущий E05 checkpoint и его границы описаны в [Modules v0.2](docs/modules-v0.2.md). Он проверяет composable representation, identity, scalar/`if`/record/bounded-sequence semantics, один root-only bounded left `fold`, owner-owned type closure, exact-outcome proof и typed witness replay. Это ещё не admission, package, runtime precondition facade, imports/helpers, nested folds или общая библиотека и не выполнение целей G01–G06.
...
- [Отчёт v0](REPORT.md), [отчёт E04](artifacts/e04/REPORT.md) и [fold v0.1 evidence](artifacts/e05/fold-v0.1-134856f/REPORT.md).
