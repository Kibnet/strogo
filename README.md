...
| **Modules v0.2 / E05 (в работе)** | Строгий composable AST для records, bounded sequences, локальных вызовов и nested `if`; canonical source → typed deterministic IR; ограниченный scalar reference evaluator | 137 parser/type/identity/reference-semantics checks; generated runtime, `fold`, proof и исполняемая библиотека ещё не готовы |

Это результаты сохранённых запусков и их отчётов: [Reserve v0](REPORT.md), [TaskGraph E04](artifacts/e04/REPORT.md) и [E05 scalar reference semantics checkpoint](artifacts/e05/REPORT.md). Они не являются результатом CI новой публичной копии. Происхождение публикации и границы сохранённых свидетельств описаны в [publication.md](docs/publication.md).
...
Текущий E05 checkpoint и его границы описаны в [Modules v0.2](docs/modules-v0.2.md). Он проверяет composable representation, identity и ограниченную исполняемую семантику scalar/`if` как reference oracle, но ещё не выдаёт generated library, proof/admission и не считается выполнением целей G01–G06.
