...
| [.NET JIT diagnostics](https://github.com/dotnet/runtime/blob/main/docs/design/coreclr/jit/viewing-jit-dumps.md) и [runtime config declarations](https://github.com/dotnet/runtime/blob/main/src/coreclr/jit/jitconfigvalues.h) | Shipped CoreCLR принимает method-filtered `DOTNET_JitDisasm`, отдельный `DOTNET_JitStdOutFile`; `JitNoInline` объявлен release config | Это диагностический канал конкретной runtime closure; наличие disassembly не доказывает корректность машинного кода |
...
`semanticDigest = H("strogo.portability-report.v0.1/semantic", canonical semantic projection)`. Projection имеет все поля report кроме `semanticDigest`, `createdAtUtc`, platform `stderrDigest` и отдельных diagnostic/performance/JIT attachments; order сохраняется как в schema. Semantic verdict включает identity input, классификацию owner domain и canonical result/refusal/termination. Runtime/JIT events, stderr, timing и memory остаются diagnostic observations и влияют на verdict только когда контракт явно делает свойство нормативным, например lazy evaluation, отсутствие effects или resource bound. Перестановка diagnostics не меняет `semanticDigest`; изменение domain classification либо canonical outcome меняет. Каждый run сохраняет canonical input/output JSONL, exit code и bounded stdout/stderr. `Unavailable` требует machine-readable причины и не считается pass. Projection показывает validation fixture/proof, target manifests, matrix rows, отсутствие contract approval/release admission и точные границы утверждения.
...
| EXEC | Обобщить proof lowering на mixed owner bundle и проверить силу инварианта | 0.98 | Exact clean-commit rerun и target translation | Зафиксировать implementation checkpoint, затем воспроизвести Linux proof на exact commit | Нет | Слабый и усиленный invariants проверены одним pinned Dafny/Linux oracle | Слабая глобальная граница истинна, но не inductive enough; усиленная prefix-length граница доказывает общий mixed source вместе с `adjust → increment` | `DafnyMixedLowering.cs`, strong/weak fixtures, proof driver |
| EXEC | Сохранить mixed proof на точном публичном коммите | 0.99 | Total wire wrapper и target profiles | Опубликовать filtered evidence checkpoint и перейти к .NET profile | Нет | Исправленный driver выполнен на clean `99415e078777bbee76e35a54a0852080c46eec00` | Два generation roots byte-equal; strong `32/0` дважды; weak `31/1` связан с `owner-prefix-invariant`; package отделяет proof evidence от ещё отсутствующей runtime portability | `artifacts/e06/mixed-proof-99415e0/**`, K-E06-013 |
| EXEC | Добавить verified total wire wrapper | 0.98 | Exact clean-commit proof и JSON adapter | Зафиксировать implementation checkpoint, затем выполнить proof на его revision | Нет | Working-tree two-root proof PASS; C# translation/build/invocation feasibility PASS | Wrapper один для обоих targets, сам проверяет logical type/requires и имеет пять owner-model assertions; adapter оставлен transport-only | `DafnyPortableWireLowering.cs`, K-E06-017/K-E06-018 |
| EXEC | Подтвердить wire wrapper на exact revision | 0.99 | JSON adapter и target package | Сохранить proof package и реализовать `dotnet-managed.v1` | Нет | Clean `9f0b2e39e9a09d778b107287ed8ae1656c5ad1b8` дал byte-equal generation, strong `42/0` дважды, weak `41/1`; внешний checkout дал conformance 56 | Proof identity/source digest теперь закреплены как общие inputs C#/Java profiles; runtime portability ещё не заявляется | `artifacts/e06/wire-proof-9f0b2e3/**`, K-E06-020/K-E06-021 |
| EXEC | Реализовать C# transport adapter и проверить одну DLL в Windows/Linux | 0.97 | Exact commit, manifest/package, full oracle/mutations | Зафиксировать checkpoint и повторить build/matrix на exact revision | Нет | Working-tree build дал byte-equal translations/records/DLL; standalone consumer прошёл 8 exact и 24 transport cases в WSL2 и Windows | Absolute translator output paths опровергли determinism; relative isolated invocation устранила drift. Profile ещё не `Portable` до оставшихся A-checks | `targets/dotnet-managed-v1/**`, target consumer, build/platform scripts, K-E06-022/K-E06-023 |
| EXEC | Закрыть escaped-surrogate totality gap C# adapter | 0.99 | Exact clean-commit rerun | Закоммитить fix и повторить обе platform lanes | Нет | Reviewer показал exception path `"\\uD800"`; working-tree DLL после fix прошла дополнительный case в Linux/Windows | Raw target surrogate остаётся `InvalidUnicode`; невалидный decoded JSON string становится `MalformedJson`; прежний `33c106b` run не используется как final evidence | `ModuleApi.cs`, target consumer, K-E06-024 |
| EXEC | Сохранить exact .NET two-platform checkpoint | 0.99 | Manifest, full owner vectors, mutations, performance/JIT | Опубликовать filtered evidence и продолжить оставшиеся .NET gates | Нет | Clean `405bd0b9…ee85b` дал byte-equal translation/record/DLL; одна DLL прошла Linux/Windows 8 exact + 24 frozen transport + 1 adversarial case | Это сильный platform checkpoint, но не финальный `Portable` до закрытия всех A-checks; runtime closure пока записана версией, не полным digest | `artifacts/e06/dotnet-checkpoint-405bd0b/**`, K-E06-025/K-E06-026 |
| EXEC | Расширить .NET run до всех owner vectors и убрать inherited consumer settings | 0.98 | Exact clean-commit rerun и projection conformance | Зафиксировать generator/consumer fix, затем повторить isolated lanes | Нет | Working-tree Linux/Windows прошли 13 owner rows; внешний вне-repo consumer выявил отсутствующий `ImplicitUsings`, single-property rerun PASS | Standalone project теперь самодостаточен; JSONL projection требует явной conformance проверки исходному fixture | invoke-vectors JSONL/generator, consumer project, K-E06-027/K-E06-028 |
| EXEC | Исключить Git revision из DLL identity | 0.99 | Exact clean-commit и no-op revision evidence | Закрепить assembly version fields и повторить build | Нет | Две DLL при одинаковых target inputs различались только потому, что SDK встраивал разные HEAD в ProductVersion | Repository revision остаётся в receipts, но не в target artifact; version `0.1.0` фиксируется profile contract | target csproj, K-E06-030 |
| EXEC | Выполнить exact .NET run с полным owner/vector set | 0.99 | Controlled cross-revision comparison, mutations, manifest/package | Закоммитить только knowledge/journal, пересобрать без target input changes и сравнить DLL | Нет | Clean `30cffa8` дал byte-equal lanes, ProductVersion `0.1.0`, Linux/Windows PASS 8+24+1+13 | Полный functional set закрыт; итоговый `Portable` всё ещё запрещён до remaining A-checks | K-E06-031/K-E06-033, local exact receipts |
| EXEC | Проверить DLL identity через docs-only revision | 0.99 | Mutations, manifest/package, runtime closure, performance/JIT | Опубликовать evidence и перейти к следующему различающему gate | Нет | Clean `30cffa8` и `ce291ca` с одинаковыми target inputs дали одну DLL `37cb02…14fc`; второй revision снова прошёл Linux/Windows full consumer | Git HEAD больше не входит в artifact bytes; repository provenance остаётся в отдельных receipts | `artifacts/e06/dotnet-full-vectors-ce291ca/**`, K-E06-034 |
| EXEC | Реализовать обязательный .NET mutation gate | 0.98 | Exact clean-commit rerun и retained evidence | Зафиксировать harness, затем повторить четыре сборки на clean revision | Нет | Working-tree baseline прошёл 4 probes; неизменённая DLL отвергнута как mutant 4/4; все 4 мутанта собрались и обнаружены: три mismatch, eager — target exception | Compile-only eager mutant и ложноположительный общий catch найдены и устранены до сохранения evidence | mutation driver/consumer/harness, K-E06-035 |
| EXEC | Подтвердить .NET mutation gate на exact revision | 0.99 | JVM A7; manifest/package, runtime closure, performance/JIT | Сохранить evidence и перейти к canonical .NET package binding | Нет | Clean `590d3dd` сохранил baseline DLL `37cb02…14fc`; full consumer PASS; 4 baseline + 4 unchanged controls; 4 compiled mutants обнаружены с exact A7 class/vector/locus | .NET-половина A7 закрыта; это не общий E06 A7 и не финальный `Portable` | `artifacts/e06/dotnet-mutations-590d3dd/**`, K-E06-036 |
| EXEC | Реализовать canonical package builder/validator | 0.98 | Exact real-DLL package и runtime closure inventory | Зафиксировать implementation, затем собрать package из retained build/proof artifacts | Нет | Windows 83, Linux 84 checks; two roots byte-equal; role/digest bindings и tamper/swap/tree/traversal/duplicate/uppercase/symlink/recomputed adapter change проверены | Expected manifest identity обязательна для Validate; synthetic entry artifact пока не является A4 evidence фактического target | `PortabilityPackage.cs`, runtime requirement fixture, K-E06-037 |
| EXEC | Воспроизвести package validator на clean revision | 0.99 | Actual DLL package и invocation-through-validation | Зафиксировать exact knowledge и добавить actual package harness | Нет | Clean `6a67a11`: Windows 83, Linux 84 с real symlink rejection; solution build 0/0 | Cross-platform format logic подтверждена, но synthetic entry не закрывает фактический A4 | K-E06-038, ignored exact reports |
| EXEC | Исправить подмену full validation proof внутренним lowering identity | 0.99 | Exact clean-commit proof/build/package run | Закоммитить исправление, затем заново выполнить proof и target build на одной revision | Нет | Review SPEC против package bytes выявил двухполевой `proof.json`; working-tree conformance после исправления 87 checks, build 0/0 | `proofDigest` теперь является domain hash полного artifact и связывает proof source/map/toolchain/closure/transcript; legacy/recomputed mutations отвергаются | `PortabilityValidationProof.cs`, K-E06-040 |
| EXEC | Запросить внешний counterexample к trusted package identity | 0.96 | Ответ участника | Учесть только воспроизводимый seam; не блокировать actual package run ожиданием | Нет | Reply #9885, ID `f82cf812-d990-4e0c-af2f-9528b5e3887a`, опубликован и прочитан обратно | Публичный checkpoint различает synthetic evidence и actual package; ошибочное JSON read-back предположение исправлено чтением HTML | K-E06-039, Posting Board thread |
| EXEC | Собрать actual .NET package и проверить его на двух ОС | 0.99 | Cross-filesystem reproducibility | Сохранить run как отрицательный checkpoint, исключить physical `ProjectDir` и повторить на clean revision | Нет | Clean `4ac7781` дал proof `42/0x2`, byte-equal sibling-root build/package, validation-before-invocation и full consumer PASS в Linux/Windows; external wrong manifest отказал до staging | DLL отличалась от прежнего mounted-root artifact при одинаковых нормативных source/toolchain inputs, поэтому package не принимается как финальное A4 evidence | `artifacts/e06/dotnet-package-4ac7781/**`, K-E06-041 |
| EXEC | Закрыть ambient build-root drift .NET target | 0.99 | Exact clean-commit cross-root rerun | Зафиксировать fix, затем повторить proof/build/package на одной clean revision | Нет | Первый working-tree fix не помог; diagnostic Csc diff выявил `/checked-` против inherited `/checked+`; второй run с pinned `/checked+` и disabled imports дал byte-equal DLL и consumer PASS; conformance `92`; structured negative diagnostic подтверждён без staging/report | Target явно фиксирует overflow checks и отключает generated editor config; build и оба platform drivers запрещают ancestor `Directory.Build.*` imports для standalone consumers; `--lane-b-root` делает различающий experiment обязательным | target csproj, build/package drivers, K-E06-041/K-E06-042 |
| EXEC | Подтвердить ambient-independent build на exact revision | 0.99 | Canonical proof closure root | Сохранить build result, исправить closure-root contract и повторить package | Нет | Clean `9ea044b`: native Linux lane и mounted repository lane дали byte-equal DLL `37cb02…14fc`; consumer и оба platform drivers PASS | Build defect закрыт; package run выявил независимый proof identity defect и не принят как final | K-E06-043/K-E06-044 |
| EXEC | Канонизировать root proof closure inventory | 0.99 | Exact clean-commit proof/package rerun | Зафиксировать direct-parent invariant и проверить rejection более широкого ancestor root | Нет | Сравнение `4ac7781`/`9ea044b` показало те же `290` files, но prefix `dafny/` изменил только `closureDigest` и затем full `proofDigest`; working-tree broader-root probe отказал до output | Exact `--dafny-root` должен быть directory самого executable, а не произвольным ancestor; expected preflight errors получают canonical `PackageHarnessRejected`; `9ea044b` package остаётся negative identity checkpoint | package harness, `artifacts/e06/dotnet-package-9ea044b/**`, K-E06-044 |
| EXEC | Сохранить canonical actual .NET package checkpoint | 0.99 | Runtime closure, JIT/performance и JVM gates | Опубликовать evidence commit и перейти к следующему различающему .NET gate | Нет | Clean `aaf2dc2`: proof `42/0x2`, canonical closure/proof identities совпали с `4ac7781`; cross-root DLL byte-equal; package roots byte-equal; exact package прошёл Linux/Windows `8+24+1+13`; wrong manifest и broader root отказали canonical без outputs | Actual A4/package и functional two-platform checkpoint принят; profile не называется `Portable` до A10–A12 и JVM/comparison | `artifacts/e06/dotnet-package-aaf2dc2/**`, K-E06-045 |
| EXEC | Связать .NET platform rows с runtime closure | 0.99 | Exact clean-commit run и absent-row aggregation | Зафиксировать implementation, затем повторить positive/corrupt/missing cases на clean revision | Нет | Working tree: Windows/Linux inventories по `191` files; разные OS-specific digests; exact expected digest принят; corrupt copy и multiversion selector дали `EnvironmentUnavailable`; conformance `107` | Пустой обязательный runtime directory/version теперь `MissingRuntimeComponent`; build consumer отделён от target `dotnet Consumer.dll`, чтобы SDK не загрязнял runtime execution claim | runtime closure, package drivers, K-E06-046 |
| EXEC | Подтвердить .NET runtime rows на exact revision | 0.99 | Absent-row aggregation, затем JIT/performance | Сохранить evidence и реализовать canonical profile report | Нет | Clean `c254468`: Windows/Linux closures по `191` files, direct consumer PASS; corrupt/missing/multiversion дали exit `70`, `EnvironmentUnavailable`, без inventory/consumer | Runtime-bound A5 и runtime-negative часть A10 приняты; A10 partial до absent mandatory row | `artifacts/e06/dotnet-runtime-c254468/**`, K-E06-047 |
| EXEC | Разделить semantic verdict и diagnostic observations | 0.99 | Full semantic projection implementation | Зафиксировать уточнение report contract и добавить digest mutation gates | Нет | Public review #9977 выявил ambiguity; ответ #9989 опубликован и прочитан обратно | Domain+canonical outcome входят в verdict; diagnostics влияют только через явную норму языка; перестановка diagnostics не меняет semantic digest | K-E06-048, public thread |
| EXEC | Реализовать completeness core platform matrix | 0.99 | Exact clean-commit matrix evidence | Зафиксировать implementation, повторить absent/complete/mix-up на clean revision | Нет | Working tree: absent Linux синтезирован `Unavailable` с `EnvironmentUnavailable`/`RowUnavailable`; две exact rows `Passed`; package mix-up отклонён до output; conformance `121` | Intermediate matrix check не присваивает `Portable`; Passed rows требуют exact manifest/package/artifact/runtime identities | report matrix, K-E06-049 |
| EXEC | Подтвердить A10 absent row на exact revision | 0.99 | A11 performance или A12 JIT | Сохранить evidence и перейти к следующему .NET gate | Нет | Clean `e222523`: absent Linux -> synthesized `Unavailable`/`NotPassed`; обе rows -> matrix `Passed`; mutated package digest -> rejection без output; conformance `121` | A10 .NET закрыт вместе с K-E06-047; matrix check остаётся ядром, а не полным report/Portable status | `artifacts/e06/dotnet-matrix-e222523/**`, K-E06-050 |
| EXEC | Реализовать package-bound .NET JIT diagnostics | 0.99 | Exact clean-commit Windows/Linux receipts | Зафиксировать implementation, повторить обе OS lanes и negative actual-log self-tests | Нет | Working tree: Windows/Linux по два `FullOpts` events после `50000` calls; decoy/missing/call-count self-tests отказали; system runtime drift дал `AmbiguousRuntimeSelection` до consumer | План symbols строится до run из validated package; observation не доказывает correctness JIT output; WSL2 не независимая машина | JIT harness, K-E06-051 |
| EXEC | Подтвердить .NET JIT rows на exact revision | 0.99 | A11 performance либо JVM profile | Сохранить evidence и подключить receipts к будущему canonical report | Нет | Clean `ca5ae50`: Windows/Linux по два process-bound `FullOpts` events после `50000` calls; actual decoy/missing logs -> `JitEvidenceMissing`, no receipt; conformance `131` | A12 закрыт для .NET, но HotSpot/JVM половина отсутствует; compilation event не является proof JIT correctness | `artifacts/e06/dotnet-jit-ca5ae50/**`, K-E06-052 |
| EXEC | Реализовать .NET A11 diagnostic на публичном ABI | 0.98 | Exact clean-commit Windows evidence и симметричный Linux driver/run | Зафиксировать implementation, повторить Windows на commit и добавить Linux lane | Нет | Working tree: `5` unique cold processes, throughput `5000 + 5 × 10000`, positive durations/ops/peak, exact runtime/package identities; conformance `136`. Preview вопроса для public review создан, но publish получил `503 Unsorted is full`, поэтому сообщения на доске нет | Замер включает JSON ABI и не делает G06 вывод. Первый driver leaked boolean output из `Environment.Remove`; cardinality validation теперь делает такой отчёт невозможным | performance consumer/driver, K-E06-053; board preview `0a6d2876-00a2-4a33-84b3-05d49b838201` не опубликован |
| EXEC | Добавить симметричный Linux A11 driver | 0.99 | Exact clean-commit two-OS rerun и retained evidence | Зафиксировать harness, затем повторить Windows/Linux на exact revision | Нет | Working tree Linux: exact package/runtime validation, `5` distinct cold PID, `5000 + 5 × 10000`, positive durations/rates/peak; conformance `138` | Оба drivers используют один public ABI consumer; controller/clock раскрыты в report. Linux выполнен в WSL2, не на независимой машине; G06 не заявляется | Linux performance driver, K-E06-054 |
| EXEC | Подтвердить A11 .NET rows на exact revision | 0.99 | JVM profile/HotSpot A12 и canonical full report | Сохранить filtered evidence и перейти к следующему общему E06 gate | Нет | Clean `2c87442`: Windows/Linux sequential runs, каждая row `5` distinct cold PID и `5000 + 5 × 10000`; positive timing/rate/peak, equal package identities/sizes; conformance `138` | A11 закрыт для .NET как diagnostic-only. Один общий host и WSL2 не дают независимости или G06; observations не входят в semantic verdict | `artifacts/e06/dotnet-performance-2c87442/**`, K-E06-055 |
| EXEC | Проверить первый JVM translation/adapter seam | 0.99 | Human-approved решение по upstream lint contradiction | Зафиксировать negative gate и подготовить amendment; не строить accepted JAR по ослабленным flags | Да | Dafny Java translation получена; strict adapter/consumer прошли `8+24+1+13` после diagnostic-only relaxed upstream compile. Normative `-Xlint:all,-cast -Werror` отверг upstream output: `67 rawtypes + 3 varargs + 1 serial` | Это фактический counterexample к §6.2.2, а не повод молча ослабить gate. До поправки JVM row — `TargetBuildRejected` | Java adapter/consumer, local normative log, K-E06-056 |
| EXEC | Выполнить E06A Stage 1 и получить exact JVM warning candidate | 0.999 | Owner approval digest `cafa0caa…c0e1` | Остановиться у human gate; после exact **«Baseline подтверждаю»** выполнить только Stage 2 E06A | Да | Clean `d21d969`: два lanes дали один baseline `cafa0caa…c0e1`, diagnostics `c18b3cf…9d2c`, exact `106` (`35/67/1/3`), empty stdout и cleanup PASS; reviewer PASS | Candidate ещё не approved и не tracked; phase 2/JAR/runtime/HotSpot не запускались | `artifacts/e06/jvm-baseline-candidate-d21d969/**`, K-E06-057, E06A Approved baseline snapshot |
| EXEC | Выполнить E06A Stage 2 после owner baseline approval | 0.999 | JAR normalizer, runtime closure и HotSpot A12 | Сохранить exact phase-2 evidence и перейти к следующему отдельному JAR gate | Да | Владелец подтвердил `Baseline подтверждаю` для `cafa0caa…c0e1`; exact generated `109` classes, strict adapter `16`, consumer `3`, Java execution `8+24+1+13`, stdout/stderr empty | Closed exclusions применены только к translator-owned sources; adapter/consumer остаются `-Xlint:all -Werror`; запуск Windows checkout `d21d969` не является независимым Linux evidence | `targets/jvm-java17-v1/upstream-warning-baseline.json`, `artifacts/e06/jvm-phase2-d21d969/**`, K-E06-065 |
| SPEC | Уточнить canonical full report contract после source/review inspection | 0.97 | Owner approval E06R и subsequent EXEC evidence | Запросить отдельное **«Спеку подтверждаю»** для E06R; после него реализовать builder без обхода E06A gate | Да | E06R прошёл independent fix-and-re-review PASS; current spec SHA-256 `89849e1d43a1c457205abd87a89cc4ecc299e75666e1a5c2329c9e9d9c68d62d` | Основной E06 §6.2.4 сохраняется за пределами explicit supersession; E06A baseline gate не меняется; reviewer sandbox writable раскрыт | `specs/2026-09-08-e06r-canonical-portability-report.md`, K-E06-060 |
| RESEARCH | Проверить предложение о нормах, выводимых из property-types | 0.98 | Минимальный контрпример от участника и отдельная SPEC перед реализацией | Сохранить только design constraint; не менять E06 semantics без новой подтверждённой SPEC | Нет | Public #10013 предложил explicit/categorical norms; ответ #10029 принял источник нормы, но отверг probabilistic verdict и потребовал canonical elaboration с rule provenance | `Pure` не означает `Total`; отсутствие единственного derivation или конфликт оставляет `OBS` и создаёт spec question | Posting Board #10013/#10029, K-E06-059 |
| RESEARCH | Запросить воспроизводимые контрпримеры после E06R | 0.98 | Ответ участника с input/mutation, expected locus/code и boundary | Разобрать только воспроизводимый seam; сначала записать знание, затем открыть отдельную SPEC/EXEC границу | Нет | Public preview/publish/read-back подтвердили seq `10049`, ID `68fe4c78-48ed-469c-b51c-04cc2696140e`, exact body `1109` UTF-8 bytes | Запрос остаётся design proposal; E06R не реализуется без отдельного **«Спеку подтверждаю»**, JVM Stage 2 не запускается без **«Baseline подтверждаю»** | Posting Board thread, K-E06-061 |
| EXEC | Реализовать E06R report builder/validator/writer | 0.98 | Same-revision real receipts and remaining E06 gates | Выполнить exact-revision integration после готовности .NET/JVM evidence; сохранить synthetic boundary | Да — только для E06A Stage 2 | Clean working-tree: solution build 0/0, Modules 362, Graph 141, portability 216; driver создал canonical `strogo.portability-report.v0.1` report | Outcome bijection, source-revision binding, cross-OS/profile OracleMismatch, semantic-vs-diagnostic digest split и staged no-overwrite writer закрыты; report остаётся validation-only и JVM не запускался | `PortabilityReportV01.cs`, `Build-PortableReport.ps1`, K-E06-062 |
| RESEARCH | Опубликовать E06R implementation checkpoint и запросить counterexample | 0.98 | Воспроизводимый внешний fixture | Разобрать ответ с exact input/mutation/locus/code; до него сохранить текущую synthetic boundary | Нет | Preview/publish/read-back подтвердили seq `10119`, ID `9ba4c9dd-d5d3-4810-ae15-201ac58f8f91`, exact body `925` UTF-8 bytes | Публичная запись не является third-party audit и не расширяет claims; JVM baseline gate сохранён | Posting Board thread, K-E06-063 |
