...
| EXEC | Связать candidate с owner outcome | 0.98 | Helper/composite contracts и admission ещё отсутствуют | Проверить strict scalar bundle, witnesses, две альтернативы и негативные proof cases | Нет | Входит в утверждённую SPEC | Отличить безопасность кандидата от соответствия owner model без вшивания одного алгоритма | owner parser/binder/evaluator, Dafny `requires`/`ensures`, reproducible harness |
| EXEC | Закрыть детерминизм отказов и resource boundary harness | 0.99 | Нет для scalar checkpoint | Пересобрать evidence и завершить независимый review | Нет | Findings независимого review исправлены в рамках утверждённой SPEC | Исключить transport-order diagnostics, unbounded output/timeout и уход descendant до kill-tree binding | structural invalid-input key, swapped regressions, suspended root + Windows Job Object, typed self-tests |
