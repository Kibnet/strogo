...
| EXEC | Добавить двухвекторную outcome coverage regression | 0.99 | End-to-end profile-validator/driver external anchor | Сохранить permutation/swap distinction и включить в следующий integration checkpoint | Нет | Нет — source review finding K-E06-069 | Whole-row permutation нормализуется; swapped `inputDigest` даёт `OutcomeCoverageMismatch` с locus `$/outcomes/v-001`; fixture сохраняет digest multiset; portability conformance `228` | `tests/Strogo.Modules.Portability.Conformance/Program.cs`, K-E06-072 |
