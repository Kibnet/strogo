...
- Связанные ссылки: [E05 SPEC](2026-09-06-composable-verified-modules-v0.2.md), [owner composite v0.3](2026-09-07-e05-owner-composite-contract-v0.3.md), [K-E05-064 и K-E05-069](../docs/knowledge-log.md), Posting Board #9554/#9556.
...
#### A/B/C discriminator обязательности invariant

Первый EXEC обязан до настройки proof heuristics построить отдельный checked-I64 sum fixture: `xs: Seq<I64,256>`, `initial=0`, owner `requires` утверждает `forall xs[i]>=0` и mathematical `seq.sum_i64(xs)<=I64_MAX`, owner/candidate step равен checked `acc+element`. Три candidate artifacts различаются только invariant AST:
...
| F-S8 | Сравнить sum fixture с invariant A=`true`, B=prefix equality + bounds, C=`false` | B=`Verified`, C=`Unproven(initial)`; A=`Verified` либо `Unproven`; normalized invariant-independent source byte-identical | generated source/diff/obligations/status report | F-AC4, F-AC5 |
...
| F-AC4 Exact fold proof | owner v0.4 quantified positive-requests domain + two alternative allocation candidates; owner step total only under owner requires passes, generated prefix without that domain fails; sum A/B/C run с одинаковыми owner/step/clauses 1–3/invariant-independent source: B=`Verified`, C=`Unproven(initial)`, A=`Verified` либо `Unproven`; parse/bind/operational/other outcome blocks | inspect prefix function/loop/all generated and agent invariants; canonical invariant-span diff + byte-equal normalized source; compare A/B/C obligations/status | Dafny report/source map/proof manifest/diff report | — |
...
- Выбран split: compiler владеет structural/termination/exact-prefix, agent всегда заполняет одно proof field (`true` допустим); A/B/C с заведомо индуктивной prefix-sum equality отдельно проверяет, даёт ли нетривиальная property prover-у дополнительную силу.
...
Итог linter: ГОТОВО; exact-snapshot Post-SPEC Review завершён с PASS.
...
### Post-SPEC Review
- Текущий статус: **PASS**; открытых BLOCKER/HIGH/MEDIUM нет. Exact reviewed whole-file snapshot до обновления этой review-записи: SHA-256 `57876B0CB79C51B4ED7C4D812ED90E0E23047B0212F577F617FC532C4601CB84`. Normative content §0–18 не менялся после verdict; его SHA-256 равен `5E399EFDF2973E1A44484BAC51F1525C7E339CECC48E7BE4786A5372947A2133` (UTF-8 bytes от начала файла до `## 19. Результат quality gate и review`, heading не включён).
...
- No-findings justification after fixes: B индуктивна через prefix-sum equality/equations; B=`Verified` и C=`Unproven(initial)` обязательны; outcome taxonomy и canonical diff fail closed; prefix op имеет closed typing/range/cost и boundary AC; dependency/external-effect scope однозначны; scenario/AC counts согласованы.
