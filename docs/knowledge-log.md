...
- Supersedes / supersededBy: добавляет root-selection invariant к K-E06-040; exact clean-commit результат сохранён в K-E06-045.

## K-E06-045

- Дата / фаза: 2026-09-08 / canonical actual .NET validation package.
- Тип / статус: Proof/package identity and two-platform execution / Confirmed on clean public commit `aaf2dc2`.
- Утверждение: clean exact run с direct-parent Dafny closure root восстановил canonical `closureDigest=4b70255b7602ac191de0101a9dce93719366deeb6f2830db46efdbcd2ef53d23` и full `proofDigest=6844985b370593862a5691064ce195d1663d15ef26d54746665acec6381df0a5`, совпадающие с `4ac7781` при тех же `290` tool files. Mixed proof дал strong `42/0x2`, weak `41/1`. Native Linux и Windows-mounted repository roots дали byte-equal checked-semantics DLL `37cb02dc8be37fe005d6515421aaba97b069168e70f52d9e8ba2ab019fe314fc`. Два package roots byte-equal; actual package identities: manifest `f947db0e3437cb98babe422c6831fa37570b04fd18a8847e12b4620a0119bb8d`, package `4dd1a36f3b430835a0b13241e7bb98a60df451a19befbbbde1801de128d8704e`, artifact set `002c89b04032d9416894f4555ff0bf279be60392ad7ee31b2521ab67840fb08a`. Один exact package прошёл validation-before-invocation и consumer `8+24+1+13` на Linux x64 и Windows x64. Wrong manifest и broader closure root дали canonical refusals с exit `1` без output artifacts.
- Scope: actual validation-only `dotnet-managed.v1` package, pinned Dafny `4.11.0`, .NET SDK `10.0.400`/runtime `10.0.11`, WSL2 Linux и Windows на одном host. Это не production admission, independent Linux CI, runtime closure digest, JIT evidence, performance result или JVM profile.
- Evidence: `artifacts/e06/dotnet-package-aaf2dc2/**`; package содержит full validation proof, source/map, proof and build inventories, adapter/public API/runtime requirement и entry DLL; `sha256.txt` связывает retained evidence files.
- Последствие: actual .NET package/proof binding, ambient-independent target build и two-platform functional path можно использовать как принятый checkpoint. `dotnet-managed.v1` всё ещё не получает итоговый статус `Portable` до оставшихся A10–A12/runtime-closure gates, а общий E06 — до JVM и comparison report.
- Supersedes / supersededBy: закрывает pending status K-E06-040/K-E06-044 и заменяет отрицательные package identity checkpoints `4ac7781`/`9ea044b` для дальнейших positive claims.

## K-E06-046

- Дата / фаза: 2026-09-08 / .NET runtime closure implementation.
- Тип / статус: Runtime identity and unavailable discrimination / Confirmed on working tree; exact clean-commit evidence pending.
- Утверждение: одного `dotnet --list-runtimes` и строки версии недостаточно для platform receipt. Runtime closure должен включать exact launcher, `host/fxr/<version>` и `shared/Microsoft.NETCore.App/<version>` с canonical logical paths, sizes и hashes; пустой обязательный каталог должен считаться отсутствующим компонентом. Несколько selectable версий в том же root также небезопасны: muxer может выбрать байты вне inventory, поэтому profile требует по одной exact версии в `host/fxr` и `shared/Microsoft.NETCore.App`. Необязательный expected digest позволил бы повреждённой среде получить новую самосогласованную identity, поэтому platform PASS требует заранее полученный OS-specific closure digest. Также `dotnet run` и проверка SDK на target смешивают consumer build с target execution и не позволяют честно отнести run только к runtime closure. Platform drivers теперь сначала проверяют expected closure, отдельно собирают consumer через pinned harness SDK и затем запускают готовый `Consumer.dll` target runtime, которому SDK не требуется.
...
- Дата / фаза: 2026-09-08 / first JVM target feasibility and stop-rule discovery.
- Тип / статус: JVM adapter behavior confirmed locally; normative target build rejected; SPEC amendment required.
- Утверждение: pinned Dafny `4.11.0` детерминированно переводит общий verified `candidate.dfy` в Java candidate/wrapper/runtime source tree, а новый dependency-free adapter `strogo.portable.v01.ModuleApi.invoke(String)` компилируется отдельно с `javac --release 17 -encoding UTF-8 -Xlint:all -Werror`. После diagnostic-only compilation upstream tree с ослабленным lint standalone Java consumer прошёл `8` exact ABI, `24` transport, lone escaped-surrogate и `13` frozen owner vectors через generated `PortableWrapper.__default.Invoke`. Однако утверждённая E06 команда для upstream translator inventory `-Xlint:all,-cast -Werror` отвергает неизменённый Dafny output: `67 rawtypes`, `3 varargs`, `1 serial`, затем `warnings found and -Werror specified`.
- Scope: local Windows Zulu OpenJDK `17.0.19`, retained verified source/package inputs и Linux Dafny translator `4.11.0`; upstream compile с `-Xlint:none` использован только для adapter feasibility и не является target artifact/evidence A4/A5/A14. JAR/normalization/package/runtime closures/mutations/HotSpot отсутствуют.
...
- Дата / фаза: 2026-09-08 / E06 full-report schema review.
- Тип / статус: Report-contract contradiction and design amendment / independent post-SPEC review `PASS`; owner approval pending.
- Утверждение: E06 §6.2.4 перечислял report fields без явных `purpose`, `contractStatus`, `admissionStatus`, source-revision anchor и receipt binding, хотя требовал показывать границы validation-only в projection. Raw JIT receipt также нельзя использовать как semantic identity: process ID, timestamps и raw log digest меняются при эквивалентном rerun. Полный report требует typed validated evidence, stable build/consumer/JIT/oracle gate summaries, canonical owner-domain outcome rows, explicit partial-state/nullability rules и staged evidence publication.
- Scope: только будущий `strogo.portability-report.v0.1` builder/validator и Markdown/evidence writer; текущий matrix, E06A baseline candidate и platform artifacts не изменены. E06R явно supersedes перечисленные clauses, остальные E06 gates сохраняет.
...
- Дата / фаза: 2026-09-08 / E06R EXEC implementation checkpoint.
- Тип / статус: Canonical report builder, validator and staged writer / Implemented and locally verified; synthetic evidence only.
- Утверждение: `PortabilityReportV01` строит immutable `strogo.portability-report.v0.1` из typed evidence, вычисляет profile/platform statuses и reasons, связывает canonical gate summaries с source revision, проверяет outcome bijection и выдаёт JSON/semantic/Markdown projections. Raw receipts, timestamps и diagnostic attachments не меняют `semanticDigest`, а outcome, cross-OS и cross-profile drift дают typed non-portable results. `PortabilityReportWriter` повторно валидирует report, сохраняет receipts/index, создаёт completion marker последним и не перезаписывает destination.
- Scope: validation-only fixed workload; report API и synthetic conformance harness. JVM artifacts и HotSpot/A12 не запускались, approved E06A baseline не создан, production admission/G05/G06 не заявляются.
